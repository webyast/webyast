...
%w(notes test makemo updatepot fetch_po rdoc pgem package release install install_policies check_syntax package-local buildrpm buildrpm-local test:test:rcov restdoc deploy_local license:report).each do |task_name|
